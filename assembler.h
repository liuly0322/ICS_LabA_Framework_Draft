...
const std::vector<std::string> kLC3TrapMachineCode(
    {"1111000000100000", "1111000000100001", "1111000000100010",
     "1111000000100011", "1111000000100100", "1111000000100101"});

enum CommandType { OPERATION, PSEUDO };

static inline void SetErrorLogMode(bool error) { gIsErrorLogMode = error; }
static inline void SetHexMode(bool hex) { gIsHexMode = hex; }
...
class assembler {
    using Commands =
        std::vector<std::tuple<unsigned, std::string, CommandType>>;
  ...
  private:
    ...
    std::string TranslateCommand(std::stringstream &command_stream,
                                 int current_address);
    std::string TranslateOprand(int current_address, std::string str,
                                int opcode_length = 3);
